...
public interface ApiEvents {
    ...
    @GET("/api/v1/events")
    Call<Events> getEventsById(@Query("id") Long[] id);

    @Headers({
            "X-Firebase-Auth: serega_mem"
    })
    @GET("/api/v1/events/instances")
    Call<EventsInstances> getInstancesByInterval(@Query("from") Long from, @Query("to") Long to);
    ...
    Call<Events> update(@Path("id") Long id, @Body DatumEvents event);
